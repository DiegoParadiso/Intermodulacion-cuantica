"""
Simular cómo dos modos vibracionales (f₁ y f₂) interactúan no linealmente, 
generando nuevos modos (f₃, f₄) mediante un circuito cuántico.
"""
...
# Circuito de intermodulación vibracional
qc.h(0)             # f₁
qc.rz(1.0, 0)       # fase de f₁
qc.h(1)             # f₂
qc.rz(2.0, 1)       # fase de f₂
qc.cx(0, 1)         # acoplamiento no lineal
qc.rz(1.5, 1)       # intermodulación → f₃, f₄
qc.measure([0, 1], cr)

# Visualización previa con simulador ideal (Statevector)
qc_no_measure = qc.remove_final_measurements(inplace=False)  # quitar mediciones para simular
state = Statevector.from_instruction(qc_no_measure)

print("\n🔍 Amplitudes complejas del estado cuántico:")
...
plot_state_city(state, title="Estado Cuántico antes de Medición")
plt.show()
...
plt.title('Intermodulación Vibracional Cuántica')
